...
# This Create method implements the C in CRUD.
    def create(self, data):
        if data is not None:
            dataCheck = self.database.animals.insert_many(data)  # data should be dictionary
...
## Enhancement Three, Databases: add options to create multiple entries at a time
##  or count the number of entries in use
# Count method to return the number of entries
    def count(self, data):
        if data is not None:
            dataCheck = self.database.animals.countDocuments(data, {"_id": False}) # data should be dictionary
        else:
            dataCheck = self.database.animals.countDocuments({}, {"_id": False})
        # return result
        return dataCheck